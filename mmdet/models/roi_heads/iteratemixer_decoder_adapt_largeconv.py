...
class IterateMixerDecoderAdaptLargeConv(CascadeRoIHead):
    _DEBUG = -1
    ...
    def __init__(self,
        ...
        super(IterateMixerDecoderAdaptLargeConv, self).__init__(
            num_stages,
            stage_loss_weights,
    ...
    def _init_layers(self):
        ...
        self.query_norm_stages = nn.ModuleList()
        ...
        for stage in range(self.num_stages):
            ...
            for s in range(SCALE):
                ...
                self.conv_generate_stages.append(Linear(self.content_dim, 9*9*self.content_dim))
    ...
    def _bbox_forward(self, stage, img_feat, query_xyzr, query_content, img_metas):
        ...
        if DEBUG:
            with torch.no_grad():
                torch.save(
                    bbox_results, 'demo/bbox_results_{}.pth'.format(IterateMixerDecoderAdaptLargeConv._DEBUG))
                IterateMixerDecoderAdaptLargeConv._DEBUG += 1
        return bbox_results
    ...
    def _update_img_feat(self, stage, img_feat, query_content):
        ...
        for s in range(SCALE):
            ...
            conv_kernel = conv_kernel.view(batchsize*self.content_dim,1,9,9)
            img_batch = img_batch.view(1, batchsize*self.content_dim, h, w)
            img_batch = F.conv2d(img_batch,conv_kernel,stride=1,padding=4, groups=batchsize*self.content_dim)
