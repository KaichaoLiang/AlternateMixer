...
class IterateMixerDecoderFFN(CascadeRoIHead):
    _DEBUG = -1
    ...
    def __init__(self,
        ...
        super(IterateMixerDecoderFFN, self).__init__(
            num_stages,
            stage_loss_weights,
    ...
    def _bbox_forward(self, stage, img_feat, query_xyzr, query_content, img_metas):
        ...
        if DEBUG:
            with torch.no_grad():
                torch.save(
                    bbox_results, 'demo/bbox_results_{}.pth'.format(IterateMixerDecoderFFN._DEBUG))
                IterateMixerDecoderFFN._DEBUG += 1
        return bbox_results
