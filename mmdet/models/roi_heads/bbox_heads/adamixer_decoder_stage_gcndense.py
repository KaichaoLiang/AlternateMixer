...
class CrossGCNDense(nn.Module):
    ...
    def forward(self, sample_points, query):
        ...
        for l in range(self.n_gcns):
            ...
            query_layer_update = torch.gather(sample_points, dim=-2, index=adjacant_index_topk_extend.view(B, N*G*self.topk,-1))*adjacant_weight_topk_extend.view(B, N*G*self.topk,-1)
