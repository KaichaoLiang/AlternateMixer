...
class CrossGCN(nn.Module):
    def __init__(self,
        ...
        self.connect_projector_l1 = nn.Linear(self.feat_dim*self.n_groups,self.feat_dim*self.outpoints*self.n_groups)
        self.connect_projector_l2 = nn.Linear(self.feat_dim*self.outpoints*self.n_groups,self.feat_dim*self.n_groups)
        
        self.matrix_projector = nn.Linear(self.feat_dim*self.n_groups, self.n_groups*self.sampled_points*self.outpoints)
        self.weight_projector = nn.Linear(self.feat_dim*self.n_groups, self.n_groups*self.feat_dim*self.feat_dim)
    ...
    def forward(self, sample_points, query):
        ...
        #===================================
        #GCN layer
        query_layer = self.connect_projector_l1(query).view(B,N,G,self.outpoints,f).contiguous()
        query_layer = query_layer.view(B,N*G,self.outpoints,f)
        query_layer = torch.layer_norm(query_layer, [query_layer.size(-2), query_layer.size(-1)])
        query_layer = self.act(query_layer)

        layer_weight = self.weight_projector(query).view(B,N,G,self.feat_dim,self.feat_dim).contiguous()
        layer_weight = layer_weight.view(B,N*G, self.feat_dim,self.feat_dim)
        ...
        #D^-1/2*X*W
        query_layer = query_layer * InvD_sqrt_trans.view(B, N*G, self.outpoints,1)
        out = out * InvD_sqrt_side.view(B, N*G, self.sampled_points,1)    
        
        #A_hat*D^-1/2*X*W
        query_layer = query_layer + torch.matmul(adjacant_weight.permute(0,1,3,2),out)
...
class AdaMixerDecoderGCNWrapperStage(BBoxHead):
    _DEBUG = -1
    ...
    @torch.no_grad()
    def init_weights(self):
        super(AdaMixerDecoderGCNWrapperStage, self).init_weights()
    ...
    def forward(self,
        ...
        AdaMixerDecoderGCNWrapperStage._DEBUG += 1
