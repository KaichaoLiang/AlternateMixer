...
from .sampling_3d_operator import sampling_3d_selfatt
...
class SelfAttSamplingMixing(BaseModule):
    _DEBUG = 0
    ...
    def __init__(self,
        ...
        super(SelfAttSamplingMixing, self).__init__()
    ...
    def forward(self, x, query_feat, query_xyzr, featmap_strides):
        ...
        if DEBUG:
            torch.save(
                sample_points_xyz, 'demo/sample_xy_{}.pth'.format(SelfAttSamplingMixing._DEBUG))

        sampled_feature, sampled_fp = sampling_3d_selfatt(sample_points_xyz, x,
                                         featmap_strides=featmap_strides,
                                         n_points=self.in_points,
                                         )
        if DEBUG:
            torch.save(
                sampled_feature, 'demo/sample_feature_{}.pth'.format(SelfAttSamplingMixing._DEBUG))
            SelfAttSamplingMixing._DEBUG += 1
...
class SelfAttMixerDecoderStage(BBoxHead):
    _DEBUG = -1
    ...
    def __init__(self,
        ...
        super(SelfAttMixerDecoderStage, self).__init__(
        ...
        self.sampling_n_mixing = SelfAttSamplingMixing(
    ...
    @torch.no_grad()
    def init_weights(self):
        super(SelfAttMixerDecoderStage, self).init_weights()
    ...
    def forward(self,
        ...
        SelfAttMixerDecoderStage._DEBUG += 1
