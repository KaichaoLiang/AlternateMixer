...
class AdaptiveSamplingMixingCatPeFFNHeadTail(BaseModule):
    _DEBUG = 0
    ...
    def __init__(self,
        ...
        super(AdaptiveSamplingMixingCatPeFFNHeadTail, self).__init__()
        ...
        self.ffn = FFN(int(content_dim/n_groups),
                       feedforward_channels=512,
                       num_ffn_fcs=2,
                       act_cfg=dict(type='ReLU', inplace=True),
                       dropout=0.0)
        self.ffn_norm = build_norm_layer(dict(type='LN'), int(content_dim/n_groups))[1]
    ...
    def init_weights(self):
        super(AdaptiveSamplingMixingCatPeFFNHeadTail, self).init_weights()
    ...
    def forward(self, x, query_feat, query_xyzr, cat_feats, cat_pes, featmap_strides):
        ...
        if DEBUG:
            torch.save(
                sample_points_xyz, 'demo/sample_xy_{}.pth'.format(AdaptiveSamplingMixingCatPeFFNHeadTail._DEBUG))

        sampled_feature, _ = sampling_3d(sample_points_xyz, x,
                                         featmap_strides=featmap_strides,
                                         n_points=self.in_points,
                                         )
        B, N, G, P, dims = sampled_feature.size()
        ...
        sampled_feature = torch.cat([cat_feats_out[:,-P:,:],cat_feats_out[:,:P,:]],dim=-1)
        sampled_feature = sampled_feature.view(B,N,G,2*P,dims)
        
        if DEBUG:
            torch.save(
                sampled_feature, 'demo/sample_feature_{}.pth'.format(AdaptiveSamplingMixingCatPeFFNHeadTail._DEBUG))
            AdaptiveSamplingMixingCatPeFFNHeadTail._DEBUG += 1
...
class AdaMixerDecoderStageCatPeFFNHeadTail(BBoxHead):
    _DEBUG = -1
    ...
    def __init__(self,
        ...
        super(AdaMixerDecoderStageCatPeFFNHeadTail, self).__init__(
        ...
        self.sampling_n_mixing = AdaptiveSamplingMixingCatPeFFNHeadTail(
    ...
    @torch.no_grad()
    def init_weights(self):
        super(AdaMixerDecoderStageCatPeFFNHeadTail, self).init_weights()
    ...
    def forward(self,
        ...
        AdaMixerDecoderStageCatPeFFNHeadTail._DEBUG += 1
