...
#================================query feature 2 feature pyramid=========================
...
def inverse_sample(sample_points, query, weight, H_feat, W_feat):
    ...
    feat_update = feat_update.permute(0,2,1).view(B,-1,H_feat,W_feat).contiguous()
    return feat_update


#================================feature pyramid sparse self-attention=========================
def sampling_3d_selfatt(
    sample_points: torch.Tensor,
    multi_lvl_values,
    featmap_strides,
    n_points: int = 1,
    ...
    for i in range(num_levels):
        ...
        out_query_lvl,  out_feat_lvl = sampling_each_level_selfatt(normalized_xy, value,
                                   weight=lvl_weights, n_points=n_points)
        out_feats.append(out_feat_lvl)
        out_query += out_query_lvl
...
def sampling_each_level_selfatt(sample_points: torch.Tensor,
                        value: torch.Tensor,
                        weight=None,
                        n_points=1):
    B1, n_queries, _t, n_groups_points, _ = sample_points.shape
    ...
    inverse_feats = selfatt_sample(sample_points, H_feat, W_feat)

    return out, inverse_feats

def selfatt_sample(sample_points, weight, H_feat, W_feat):
    B, Nq, Np, _ =sample_points.size()
    sample_points=sample_points.long()
    sample_points[:,:,:,0] = torch.clip(sample_points[:,:,:,0], 0, H_feat-1)
    sample_points[:,:,:,1] = torch.clip(sample_points[:,:,:,1], 0, W_feat-1)
