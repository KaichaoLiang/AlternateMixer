#-------------------------------------------------------------------------
# file: iteratemixer_decoder_withfcos.py
# author: kaichao liang
# date: 2022.06.27
# discription: alternative iteration for feature paramid and queries
# fcos_head added for loss constraint, the predicted boxes can be generated from either Mixer or FcosMixer.
...
class IterateMixerDecoderFcos(CascadeRoIHead):
    _DEBUG = -1
    ...
    def __init__(self,
        ...
        super(IterateMixerDecoderFcos, self).__init__(
            num_stages,
            stage_loss_weights,
    ...
    def _init_layers(self):
        ...
        self.query_activate_stages = nn.ModuleList()
    ...
    def _bbox_forward(self, stage, img_feat, query_xyzr, query_content, img_metas):
        ...
        if DEBUG:
            with torch.no_grad():
                torch.save(
                    bbox_results, 'demo/bbox_results_{}.pth'.format(IterateMixerDecoderFcos._DEBUG))
                IterateMixerDecoderFcos._DEBUG += 1
        return bbox_results
    ...
    def forward_train(self,
        ...
        loss_fcos = self.fcos_head.forward_train(x, img_metas, gt_bboxes, gt_labels, gt_bboxes_ignore)
        all_stage_loss['fcos'] = loss_fcos
    ...
    def simple_test(self,
        ...
        if(not self.from_fcos):
            ...
            return self.fcos_head.simple_test(x, img_metas, rescale)
