...
class AdaMixerDecoderCatPe(CascadeRoIHead):
    _DEBUG = -1
    ...
    def __init__(self,
        ...
        super(AdaMixerDecoderCatPe, self).__init__(
            num_stages,
            stage_loss_weights,
    ...
    def _bbox_forward(self, stage, img_feat, query_xyzr, query_content, cat_feats,cat_pes,img_metas):
        num_imgs = len(img_metas)
        bbox_head = self.bbox_head[stage]

        cls_score, delta_xyzr, query_content, cat_feats, cat_pes = bbox_head(img_feat, query_xyzr,
                                                         query_content,
                                                         cat_feats,
                                                         cat_pes,
        ...
        if DEBUG:
            with torch.no_grad():
                torch.save(
                    bbox_results, 'demo/bbox_results_{}.pth'.format(AdaMixerDecoderCatPe._DEBUG))
                AdaMixerDecoderCatPe._DEBUG += 1
        return bbox_results
    ...
    def forward_train(self,
                      ...
                      imgs_whwh=None,
                      gt_masks=None):
        # History selected features concatnation
        cat_feats = None
        cat_pes = None
        ...
        for stage in range(self.num_stages):
            bbox_results = self._bbox_forward(stage, x, query_xyzr, query_content,cat_feats,cat_pes,
                                              img_metas)
            ...
            #cat_feats update
            cat_feats = bbox_results['cat_feats']
            cat_pes = bbox_results['cat_pes']
    ...
    def simple_test(self,
        ...
        for stage in range(self.num_stages):
            bbox_results = self._bbox_forward(
                stage, x, query_xyzr, query_content, cat_feats, cat_pes, img_metas)
            query_content = bbox_results['query_content']
            cls_score = bbox_results['cls_score']
            #cat_feats update
            cat_feats = bbox_results['cat_feats']
